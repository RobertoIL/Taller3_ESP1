...
export class Player {
  ...
  update(input) {
    ...
    if (
      ((!input.includes(this.keys.up) && !input.includes(this.keys.down)) ||
        (input.includes(this.keys.up) && input.includes(this.keys.down))) &&
      this.ySpeed != 0
    ) {
      if (this.ySpeed < 0) {
    ...
    if (
      ((!input.includes(this.keys.left) && !input.includes(this.keys.rigth)) ||
        (input.includes(this.keys.left) && input.includes(this.keys.rigth))) &&
      this.xSpeed != 0
    ) {
      if (this.xSpeed < 0) {
  ...
  drawCanvas(context, X, Y) {
    if (this.playerN == 1) {
      ...
      context.fillStyle = "black";
      context.fillRect(
        this.game.width - 10 - this.totalLife * 3,
        25,
        this.totalLife,
        25
      );
      ...
      context.fillText(
        "Bastian",
        this.game.width - 5 - this.totalLife * 3,
        42.5,
        this.totalLife * 3
      );
